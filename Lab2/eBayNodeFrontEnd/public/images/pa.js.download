;(function () {
  ...
  PAYPAL.analytics.Analytics.prototype = {
    ...
    _createBeacon: function (options) {
      ...
      if (typeof options.onBeaconCreate === 'function' && options.onBeaconCreate(beacon) !== false) {
        ...
        beacon.src = this._generateBeaconUrlForLoggingActivity(options);
      }
      PAYPAL.analytics.beaconURL = beacon.src; // expose the parameters for testing purposes
    },
    ...
    _generateBeaconUrl: function (options) {
      ...
      for (var opt in options.data) {
        ...
        if (opt === 'opic') {
          var opic = options.data[opt] || tempFpti[opt];
          opic = opic.replace(/\w+@\w+\.\w+/g, '');
          opic = opic.replace(/\d+/g, '');
          tempFpti[opt] = opic;
        }
        else {
          tempFpti[opt] = options.data[opt] || tempFpti[opt];
        }
      }
      ...
      // If site catalyst object exists add it to the parameters
      if (typeof s === 'object' && typeof s.beacon_url !== 'undefined' && s.beacon_url !== '') {
        if (s.beacon_url.split('?')[1]) {
          var scParam = s.beacon_url.split('?')[1];
          if (scParam.split('&')) {
            var scParamArr = scParam.split('&');
            var scParamJson = {};
            for (var i = 0; i < scParamArr.length; i++) {
              var bits = scParamArr[i].split('=');
              scParamJson[bits[0]] = bits[1];
            }
            scParamJson = JSON.stringify(scParamJson);
            var backUpUrl = url;
            var limit = 8000;
            url = url + '&scParams=' + scParamJson;
            if (url.length > limit) {
              url = backUpUrl;
            }
          }
    ...
    _getPluginDataIE: function () {
      ...
      for (var i = 0; i < components.length; i++) {
        var ver = body.getComponentVersion('{' + components[i] + '}', 'componentid');
        var name = names[i];
        if (ver) {
          plugins[name] = ver;
        }
  ...
  PAYPAL.analytics.Analytics.prototype.utils = {
    ...
    getTargetAttr: function (target, primaryAttrName) {
      ...
      if (!attr) {
        attr = this.getFirstText(target) || target.getAttribute('name') || target.getAttribute('id') || target.getAttribute('class') || target.getAttribute('href') || target.getAttribute('alt');
      }
      return attr;
    },
  ...
  PAYPAL.analytics.setup.init = function (options) {
    ...
    window.fptiserverurl = options.url || window.fptiserverurl;
    ...
    if (options.trackPPClicks) {
      ...
      analytics.trackClicks({
        elements: paUtils.wrapSelectors('*[data-pa-click]', options.wrappingElement),
        onClick: function (ev) {
          var target = ev.currentTarget || ev.srcElement;
    ...
    if (options.trackPPExitClicks) {
      ...
      analytics.trackClicks({
        elements: paUtils.wrapSelectors('*[data-pa-exit]', options.wrappingElement),
        onClick: function (ev) {
          var target = ev.currentTarget || ev.srcElement;
    ...
    if (options.trackPPDownloadClicks) {
      ...
      analytics.trackClicks({
        elements: paUtils.wrapSelectors('*[data-pa-download], *[href*=".zip"], *[href*=".wav"], *[href*=".mov"], *[href*=".mpg"], *[href*=".avi"], *[href*=".wmv"], *[href*=".doc"], *[href*=".docx"], *[href*=".pdf"], *[href*=".xls"], *[href*=".xlsx"], *[href*=".ppt"], *[href*=".pptx"], *[href*=".txt"], *[href*=".csv"], *[href*=".psd"], *[href*=".tar"]', options.wrappingElement),
        onClick: function (ev) {
          var target = ev.currentTarget || ev.srcElement;
    ...
    if (options.trackPPLegacyClicks) {
      ...
      analytics.trackClicks({
        elements: paUtils.wrapSelectors('*[class*="scTrack"]', options.wrappingElement),
        onClick: function (ev) {
          var target = ev.currentTarget || ev.srcElement;
          var link = paUtils.getTargetAttr(target);
          var pldata = getPageLinkData(link);
    ...
    if (options.trackPPLegacyExitClicks) {
      ...
      analytics.trackClicks({
        elements: paUtils.wrapSelectors('*[class*="scExit"]', options.wrappingElement),
        onClick: function (ev) {
          var target = ev.currentTarget || ev.srcElement;
          var link = paUtils.getTargetAttr(target);
          var pldata = getPageLinkData(link);
