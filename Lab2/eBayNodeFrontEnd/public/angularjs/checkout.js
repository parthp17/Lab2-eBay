...
checkout.controller('checkout', function($scope, $http, $window) {
	...
	$scope.init = function()
	{
		...
		$http({
			...
			{
				$scope.details.push(data.result.adrLine1);
				$scope.details.push(data.result.adrLine2);
				$scope.details.push(data.result.city);
				$scope.details.push(data.result.state);
				$scope.details.push(data.result.zipcode);
				$scope.details.push(data.result.contact);
			}
			else
			{
				console.log(data);
